...
port_number = os.getenv("port", "3000")  # .env에서 port 값을 가져오거나 기본값 3000 사용
...
def get_data():
    try:
        # 1. 쿼리 조건 설정 (qs가 해석할 수 있는 형식)
        # 예: status가 'active'이고, level이 5 이상인 데이터
        query_params = {
            "_id": "arbiter",
            # "level[$gte]": 5,
        }
        ...
        if response.status_code == 200:
            ...
            print(f"❌ 서버 오류 상세: {response.text=}")
